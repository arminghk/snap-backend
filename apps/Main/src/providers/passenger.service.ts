...
PassengersService
  ...
  async authorize({
    ...
    const decodedToken: any =
      this.utils.JwtHandler.decodeToken(token);
    ...
    if (decodedToken) {
      ...
      if (passenger) {
        ...
        if (+new Date(decodedToken.refreshExpiresAt) <= now) {
          ...
          if (session) {
            const accessToken =
              new this.utils.JwtHandler.AccessToken(
                passenger.id,
                'PASSENGER',
              );
  ...
  private async getPassengerById(id: string) {
    let passenger = null;
    let _passenger: any = await this.redis.cacheCli.get(
      `passenger_${id}`,
    );
  ...
  private async getSessionById(id: string) {
    let session = null;
    let _session: any = await this.redis.cacheCli.get(
      `passengerSession_${id}`,
    );

    if (!_session) {
      _session =
        await this.pg.models.PassengerSession.findByPk(id);
  ...
  private async extendSession(
    ...
    const updated =
      await this.pg.models.PassengerSession.update(
        { refreshExpiresAt },
